    ...
    public partial class TestForm : Form
    {
        ...
        private void GetStatueTimer_Tick(object sender, EventArgs e)
        {
            ...
            catch (Exception)
            {
                ...
                MovePar(3,serialPortX);
                MovePar(3, serialPortZ);
        ...
        private void DoTestIoStatusThr()
        {
            while(blTestPortOneIoStatus)//串口打开之后，便一直执行检测线程
            {
                if(blMotorIsInpos)
                {
                    MovePar(7, serialPortX);//如果运动到检测的位置 则停止运动
        ...
        private void MovePar(int dir,SerialPort sp)
        {
            String Vel = "";
            if (!sp.IsOpen) //如果没打开
            {
            ...
            string[] strMovePar = GetProtocolPar(Vel,dir,sp);
            if (strMovePar != null)
            {
                SendRSData(strMovePar,sp);
            }
            else
            {
        ...
        //-----发送数据-----------------
        private void SendRSData(string[] strMovePar)
        {
            //处理数字转换
            string[] strArray = strMovePar;
            int byteBufferCnt = 0;
            int byteBufferLength = strArray.Length;
            byte[] byteBuffer = new byte[byteBufferLength];
            for (int i = 0; i < strArray.Length; i++)
        ...
        private void SendZRSData(string[] strMovePar)
        {          
            //处理数字转换
            string[] strArray = strMovePar;
            int byteBufferCnt = 0;
        ...
        //-----发送数据-----------------
        private void SendRSData(string[] strMovePar, SerialPort serialport)
        {
            //处理数字转换
            string[] strArray = strMovePar;
            int byteBufferCnt = 0;
        ...
        private int testXAxisSensorCnt = 0;
        private void button1_Click(object sender, EventArgs e)
        {
            if(testXAxisSensorCnt == 0)
            {
                MovePar(1, serialPortX);
                testXAxisSensorCnt++;
                blInfSensor = false;
        ...
        private void testPosOnly()
        {
           while(blTestPos)
            {                       
               if(blInfSensor && testXAxisSensorCnt == 1)
                {
                    MovePar(3, serialPortX);//停止运动
                    blInfSensor = false;
                    testXAxisSensorCnt = 0;
                }  
        ...
        private Thread testInitThr;
        private bool bltestInit=false;
        private void button2_Click(object sender, EventArgs e)
        {
            if(!serialPortX.IsOpen)
            {
                MessageBox.Show("请打开X轴串口！");
                return;
            }
            if (!serialPortZ.IsOpen)
            {
                MessageBox.Show("请打开Z轴串口！");
                return;
            }
            MovePar(0, serialPortZ);
            MovePar(0, serialPortX);
            ...
            while (bltestInit)
            {
                if (btnPosLimit.BackColor == Color.Red && btnZPosLimit.BackColor == Color.Red)
                {
                    bltestInit = false;
                    if (testXAxisSensorCnt == 0)
                    {
