...
namespace IdentitySys
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            textBox_IP.Text = "169.254.255.75";
        }
        ...
        {
            StringBuilder ip = new StringBuilder("169.254.76.124");
        ...
        private void ReadSEQCount_Click(object sender, EventArgs e)
        {
            int moduleIndex = Convert.ToInt32(textBox1.Text); //获取通道
            int nCount = -1;
            long lRet = OPTController.ReadStepCount(moduleIndex, ref nCount);
            if (lRet == 0)
            {
                textBox_Status.Text = "Read SEQ count successfully!";
            }
            else
            {
                textBox_Status.Text = "Failed to read SEQ count";
            }
        }

        private void ReadCurStepIndex_Click(object sender, EventArgs e)
        {
            int moduleIndex = Convert.ToInt32(textBox1.Text); //获取通道
            int nIndex = -1;
            long lRet = OPTController.ReadCurrentStepIndex(moduleIndex, ref nIndex);
            if (lRet == 0)
            {
                textBox_Status.Text = "Read SEQ current index successfully!";
            }
            else
            {
                textBox_Status.Text = "Failed to read SEQ current index";
            }
        }
    }
}
